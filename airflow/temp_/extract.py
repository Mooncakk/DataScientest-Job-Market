...
def get_access_token():
        ...
        config.read(f'{AIRFLOW_HOME}/data/france_travail_ressources/api_key/config')
...
def france_travail_to_CSV2(csv_name=f'offres{date.today()}.csv'):
    ...
    types_contrat = []
    with open(f'{AIRFLOW_HOME}/data/france_travail_ressources/referentiels/type_contrat.csv', mode='r', encoding='utf-8') as csvfile:
    ...
    codes_rome = []
    with open(f'{AIRFLOW_HOME}/data/france_travail_ressources/referentiels/metier.csv', mode='r', encoding='utf-8') as csvfile:
        reader = csv.DictReader(csvfile)
        for row in reader:
...
def france_travail_to_CSV2_in_container(csv_name=f'{AIRFLOW_HOME}/data/to_ingest/bronzea/france_travail_{date.today()}.csv'):
    """ Génère un fichier CSV contenant les offres à partir des fichiers CSV existants."""
...
def load_commune_to_csv():
    ...
    rows = [
        ...
        {'code_commune': '01039', 'code_postal': 1350, 'libelle': '01039', 'code_departement': '01'}
    ]
...
def main():
        #conn = postgres_bdd_auth(database="jobmarket", host="localhost", user="postgres", password="postgres", port="5432")
        france_travail_to_CSV2_in_container()
